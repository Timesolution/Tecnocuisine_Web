...
{
    public partial class Retenciones : Page
    {
        ...
        ControladorCuentaRetenciones controladorCuentaRetenciones = new ControladorCuentaRetenciones();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                ObtenerTodosLasCuentaRetenciones();
            }
            CargarClientes();
        }
        public void ObtenerTodosLasCuentaRetenciones()
        {
            try
            {
                var cuenta = controladorCuentaRetenciones.ObtenerTodosCuentaRetenciones();
                if (cuenta != null)
                {
                    foreach (Tecnocuisine_API.Entitys.Retenciones item in cuenta)
                    {
                        RellenarTabla(item);
                    }
                }
        ...
        public void FiltrarVentas(int idCli, string FechaD, string FechaH)
        {
            try
            {
                ...
                var dt = controladorCuentaRetenciones.FiltrarCuentaRetenciones(FechaD, FechaH, idCli);
                decimal total = 0;
                foreach (DataRow row in dt.Rows)
                {
                    Tecnocuisine_API.Entitys.Retenciones cc = new Tecnocuisine_API.Entitys.Retenciones();
                    cc.id = Convert.ToInt32(row["id"]);
                    cc.fecha = ((DateTime)row["fecha"]);
                    cc.Tipo = row["Tipo"].ToString();
                    cc.Documento = row["Documento"].ToString();
        ...
        public void RellenarTabla(Tecnocuisine_API.Entitys.Retenciones cc)
        {
            try
            {
                //fila
                ...
                celDocumento.Width = Unit.Percentage(10);
                celDocumento.Text = cc.Documento;
                celDocumento.VerticalAlign = VerticalAlign.Middle;
                celDocumento.HorizontalAlign = HorizontalAlign.Left;
                celDocumento.Attributes.Add("style", "padding-bottom: 0px !important; padding-top:   0px; vertical-align: middle;");
