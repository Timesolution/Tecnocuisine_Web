...
namespace Tecnocuisine.Compras
{
    public partial class Pagos : Page
    {
        ...
        public void FiltrarVentas(int idCli)
        {
            try
            {
                Tecnocuisine_API.Entitys.Proveedores proveedores = cp.ObtenerProveedorByID(idCli);
                AliasCliente.Value = proveedores == null ? "No se encontro Proveedor" : proveedores.Id + " - " + proveedores.Alias;
              
                var dt = controladorCuentaCorriente.FiltrarCuentaCorrienteSinFechas(idCli);
                decimal total = 0;
                foreach (DataRow row in dt.Rows)
                {
                    string desc = row["Descripcion"].ToString();
                    decimal sald = Convert.ToDecimal(row["Saldo"]);
                    if (sald != 0.00M && (!desc.Contains("Recibo") || !desc.Contains("Pago")))
                    {
                    ...
                    cc.idFactura = Convert.ToInt32(row["idFactura"]);
